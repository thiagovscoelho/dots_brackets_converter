...
Dots are read and written according to [the rules in](https://plato.stanford.edu/entries/pm-notation/dots.html) Russell and Whitehead’s [*Principia Mathematica*](https://en.wikipedia.org/wiki/Principia_Mathematica) (hereafter “PM”), which were also used by other authors such as [Quine](https://www.hup.harvard.edu/books/9780674554511), Landini ([2012](https://link.springer.com/book/10.1057/9780230360150), [2021](https://link.springer.com/book/10.1007/978-3-030-66356-8)), and [Carnap](https://richardzach.org/2006/07/dots-as-brackets-in-formulas/). Roughly, these rules use larger groups of dots to indicate higher levels of bracketing, so for instance, `a → b .→. c` means `(a → b) → c`. (Support for the simpler rule used by [Church](https://archive.org/details/introductiontoma0000chur), [Robbin](https://archive.org/details/mathematicallogi0000robb), and [Chwistek](https://quod.lib.umich.edu/u/umhistmath/AAS7985.0001.001/?view=toc) is planned.) Dots-to-brackets conversion is tolerant of all these style variants, but brackets-to-dots outputs a style closest to Landini’s. (See below for details.)
...
+ **Definite description (iota):** Can be used as a quantifier, as in `((ιx)(ϕx))ψ`, or as an atom, as in `(ιx)(ϕx) + B`. Currently, the inside of an iota expression is left unchanged, so e.g., if you write `(ιx)(x.|.z+c)` (probably intending it to be read as `(ιx)(x|(z+c))`), this will be left unchanged in the dots-to-brackets output. (These expressions are not very common, and are supported for the sake of interpreting their use in PM, where they are usually not very internally complex. Nevertheless, I welcome pull requests to improve this behavior, as long as they don’t break other functionality.)
