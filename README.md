...
To test the program, try converting the tautology `((¬c ⊃ b) ⊃ ((b ⊃ a) ⊃ ((c ⊃ a) ⊃ a))) ⊃ ((d ⊃ (¬c ⊃ b)) ⊃ ((b ⊃ a) ⊃ ((c ⊃ a) ⊃ (d ⊃ a))))` from bracket form to dot form, which should give you `¬c :⊃: b .:⊃:. b .⊃. a :⊃: c ⊃ a .⊃. a ::⊃:: d :⊃: ¬c .⊃. b .:⊃:. b .⊃. a :⊃: c ⊃ a .⊃. d ⊃ a` in PM mode, or `((¬c ⊃ b) ⊃ . (b ⊃ a) ⊃ . (c ⊃ a) ⊃ a) ⊃ . (d ⊃ . ¬c ⊃ b) ⊃ . (b ⊃ a) ⊃ . (c ⊃ a) ⊃ . d ⊃ a` in Church mode. Converting the dot form to bracket form will also work.
...
***Iota expressions (definite descriptions).*** Currently, the inside of an iota expression is left unchanged, so e.g., if you write `(ιx)(x.|.z+c)` (probably intending it to be read as `(ιx)(x|(z+c))`), this will be left unchanged in the dots-to-brackets output. (These expressions are not very common, and are supported for the sake of interpreting their use in PM, where they are usually not very internally complex. Nevertheless, I welcome pull requests to improve this behavior, as long as they don’t break other functionality.)
