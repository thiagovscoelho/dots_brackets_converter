...
+ **Definite description (iota):** Can be used as a quantifier, as in `((ιx)(ϕx))ψ`, or as an atom, as in `(ιx)(ϕx) + B`. Currently, the inside of an iota expression is left unchanged, e.g., if you write `(ιx)(x.|.z+c)` (probably intending it to be read as `(ιx)(x|(z+c))`), this will be left unchanged in the dots-to-brackets output. (These expressions are not very common, and are supported for the sake of interpreting their use in PM, where they are usually not very internally complex.)
