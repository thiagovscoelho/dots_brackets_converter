...
export function lexer(input, notation, andOp = "∧") {
...
export function attachBP(tokens, notation){
...
export function parse(tokens, notation){
  let pos=0;
  ...
  function nud(tok){
    switch(tok.type){
      ...
      case "IotaQuant": {
        // Use the notation parameter rather than a global
        const innerToks = lexer(tok.desc, notation);
        attachBP(innerToks, notation);
        const descAst   = parse(innerToks, notation);
...
export function toBrackets(node, root=true){
  switch(node.type){
...
export function annotateDepth(node,d=0){ if(!node)return; if(node.type==="Binary"){ node.depth=d; annotateDepth(node.left,d+1); annotateDepth(node.right,d+1);} else if(node.child){ annotateDepth(node.child,d);} if(node.desc) annotateDepth(node.desc,d); }
export function assignWeights(node){ const arr=[]; walkBinary(node,n=>arr.push(n)); const max=arr.length?Math.max(...arr.map(n=>n.depth)):0; arr.forEach(n=>n.weight=max-n.depth); }

export function toDots(node, dotNeg=false, strict=false, dotQuant=false, strictQuant=false){
  switch(node.type){
    case "Atom": return node.value;
    case "Not": {
